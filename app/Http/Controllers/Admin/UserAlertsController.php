...
class UserAlertsController extends Controller
{
    ...
    public function edit(UserAlert $userAlert)
    {
        ...
        $users = User::pluck('name', 'id');
        return view('admin.userAlerts.edit',compact('userAlert','users'));
    }

    public function update(Request $request, UserAlert $userAlert)
    {
        $data = [];
        $inputs = $request->all();
        $inputs['type'] = implode(", ", $inputs['type']);
        $userAlert->update($inputs);
        // 

        if(in_array('1', $request->type)){
            \DB::table('user_user_alert')->where('user_alert_id', $userAlert->id)->delete();
            $userAlert->users()->sync($request->input('users', []));
