class FibonacciTree {
    ...
    handleMouseOver(t) {
        t.data.isPlaceholder || (this.tooltip.transition().duration(200).style("opacity", .9), this.tooltip.html(`${t.data.data.email}<br/>${t.data.data.accountNum}`).style("left", `${d3.event.pageX-300}px`).style("top", `${d3.event.pageY-200}px`))
    }
    handleMouseOut(t) {
