...
@can('cover_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.covers.create') }}">
                    ...
                    @foreach($covers as $key => $cover)
                        <tr data-entry-id="{{ $cover->id }}">
                            ...
                            <td>
                                ...
                                @can('cover_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.covers.edit', $cover->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('cover_delete')
                                    <form action="{{ route('admin.covers.destroy', $cover->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cover_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.covers.massDestroy') }}",
