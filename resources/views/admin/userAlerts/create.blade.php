...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-alerts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="show_hide">{{ trans('cruds.userAlert.fields.show_hide') }}</label>
                <select class="form-control {{ $errors->has('show_hide') ? 'is-invalid' : '' }}" name="show_hide" id="show_hide">
                    ...
                    @foreach(App\Models\UserAlert::SHOW_HIDE as $key => $label)
                        <option value="{{ $key }}" {{ old('show_hide', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type[]" id="type" multiple>
                    @foreach(App\Models\UserAlert::TYPE as $id => $lable)
                        <option value="{{ $id }}" {{ in_array($id, old('type', [])) ? 'selected' : '' }}>{{ $lable }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
