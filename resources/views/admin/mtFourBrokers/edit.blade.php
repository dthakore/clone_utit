...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.mtFourBroker.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mt-four-brokers.update", [$mtFourBroker->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.mtFourBroker.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $mtFourBroker->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('server_login') ? 'is-invalid' : '' }}" type="number" name="server_login" id="server_login" value="{{ old('server_login', $mtFourBroker->server_login) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('server_password') ? 'is-invalid' : '' }}" type="text" name="server_password" id="server_password" value="{{ old('server_password', $mtFourBroker->server_password) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('server_address') ? 'is-invalid' : '' }}" type="text" name="server_address" id="server_address" value="{{ old('server_address', $mtFourBroker->server_address) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('server_port') ? 'is-invalid' : '' }}" type="number" name="server_port" id="server_port" value="{{ old('server_port', $mtFourBroker->server_port) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agent') ? 'is-invalid' : '' }}" type="number" name="agent" id="agent" value="{{ old('agent', $mtFourBroker->agent) }}" step="1">
                @if($errors->has('agent'))
                    <div class="invalid-feedback">
                        {{ $errors->first('agent') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text" name="location" id="location" value="{{ old('location', $mtFourBroker->location) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\MtFourBroker::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $mtFourBroker->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('comment') ? 'is-invalid' : '' }}" name="comment" id="comment">{{ old('comment', $mtFourBroker->comment) }}</textarea>
