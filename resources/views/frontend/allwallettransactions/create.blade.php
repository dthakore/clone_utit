        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.allwallettransaction.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.allwallettransactions.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="transaction_type" id="transaction_type">
                                ...
                                @foreach(App\Models\Allwallettransaction::TRANSACTION_TYPE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('transaction_type', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="reference_num" id="reference_num" value="{{ old('reference_num', '') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="transaction_comment" id="transaction_comment">{{ old('transaction_comment') }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="transaction_status" id="transaction_status">
                                ...
                                @foreach(App\Models\Allwallettransaction::TRANSACTION_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('transaction_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
