        ...
        <div class="col-md-12">
            @can('user_exchange_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-exchanges.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $userExchange->id }}">
                                        ...
                                        <td>
                                            @can('user_exchange_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-exchanges.show', $userExchange->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_exchange_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-exchanges.edit', $userExchange->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_exchange_delete')
                                                <form action="{{ route('frontend.user-exchanges.destroy', $userExchange->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_exchange_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-exchanges.massDestroy') }}",
