        ...
        <div class="col-md-12">
            @can('rank_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.ranks.create') }}">
                            ...
                            <tbody>
                                @foreach($ranks as $key => $rank)
                                    <tr data-entry-id="{{ $rank->id }}">
                                        ...
                                        <td>
                                            @can('rank_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.ranks.show', $rank->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('rank_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.ranks.edit', $rank->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('rank_delete')
                                                <form action="{{ route('frontend.ranks.destroy', $rank->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('rank_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.ranks.massDestroy') }}",
