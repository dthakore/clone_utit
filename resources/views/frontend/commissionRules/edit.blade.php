    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.commission-rules.update", [$commissionRule->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\CommissionRule::AMOUNT_TYPE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="amount_type_{{ $key }}" name="amount_type" value="{{ $key }}" {{ old('amount_type', $commissionRule->amount_type) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="amount" id="amount" value="{{ old('amount', $commissionRule->amount) }}" step="0.00001" required>
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\CommissionRule::WALLET_STATUS_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="wallet_status_{{ $key }}" name="wallet_status" value="{{ $key }}" {{ old('wallet_status', $commissionRule->wallet_status) === (string) $key ? 'checked' : '' }}>
