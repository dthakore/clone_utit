<nav class="main-header side-header sticky nav nav-item navbar navbar-expand-md navbar-light bg-white shadow-sm">
    <a class="navbar-brand" href="{{ url('/') }}">
        <img src="{{ asset('/storage/images/logo.png') }}" class="" width="150px"/>
    </a>
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                ...
                @if($host == 'cashback.utradeitrade.com')
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('frontend.indexCashback',['id' => isset($url_user_id)? $url_user_id : '' ]) }}">
                            {{ __('Dashboard') }}
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('frontend.account',['id' => isset($url_user_id)? $url_user_id : '' ]) }}">
                            {{ __('My Account') }}
                        </a>
                    </li>
                    <li class="nav-item ">
...
{{--                    </li>--}}
                    ...
                    <li class="nav-item {{ request()->routeIs('frontend.social') ? 'active' : '' }}">
                        <a class="nav-link" href="#">
                            {{ __('Socials') }}
                        </a>
                    </li>
                    <li class="nav-item {{ request()->routeIs('frontend.affiliate') ? 'active' : '' }}">
                        <a class="nav-link" href="{{ route('frontend.affiliate') }}">
                            {{ __('Affiliate Center') }}
                        </a>
                    </li>
                    {{--@can('bot_management_access')--}}
                    {{--<li class="nav-item dropdown {{ request()->is("exchanges*") ? "active" : "" }} {{ request()->is("user-exchanges*") ? "active" : "" }} {{ request()->is("symbols*") ? "active" : "" }} {{ request()->is("bots*") ? "active" : "" }} {{ request()->is("sessions*") ? "active" : "" }} {{ request()->is("trades*") ? "active" : "" }} {{ request()->is("covers*") ? "active" : "" }}">--}}
                        {{--<a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink2" data-toggle="dropdown" aria-haspopup="true"--}}
                           {{--aria-expanded="false">{{ trans('cruds.botManagement.title') }}</a>--}}
                        {{--<div class="dropdown-menu dropdown-primary" aria-labelledby="navbarDropdownMenuLink2">--}}
                            {{--<a class="dropdown-item {{ request()->is("user-exchanges*") ? "active" : "" }}" href="{{ route('frontend.user-exchanges.index') }}">{{ trans('cruds.userExchange.title') }}</a>--}}
                            {{--<a class="dropdown-item {{ request()->is("bots*") ? "active" : "" }}" href="{{ route('frontend.bots.index') }}">{{ trans('cruds.bot.title') }}</a>--}}
                        {{--</div>--}}
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu dropdown-primary" aria-labelledby="navbarDropdownMenuLink2">
                            <a class="dropdown-item {{ request()->is("profile*") ? "active" : "" }}" href="{{ route('frontend.profile.index') }}">My Profile</a>
                            <a class="dropdown-item {{ request()->is("account*") ? "active" : "" }}" href="{{ route('frontend.account',['id' => base64_encode(auth()->user()->id)]) }}">My Account</a>
                            <a class="dropdown-item {{ request()->is("order*") ? "active" : "" }}" href="{{ route('frontend.order') }}">My Orders</a>
                            {{-- <a class="dropdown-item {{ request()->is("license*") ? "active" : "" }}" href="{{ route('frontend.license') }}">My License</a> --}}
                            {{--<a class="dropdown-item {{ request()->is("wallet*") ? "active" : "" }}" href="{{ route('frontend.wallet') }}">My Wallet</a>--}}
                            <a class="dropdown-item {{ request()->is("password*") ? "active" : "" }}" href="{{ route('frontend.password') }}">Change Password</a>
                            @if (auth()->user()->is_admin)
                                <a class="dropdown-item" href="{{ url('/admin') }}">Go to Admin</a>
                            @endif
                            <a class="dropdown-item" href="{{ route('sio.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Logout</a>
                        </div>
                        <form id="logout-form" action="{{ route('sio.logout') }}" method="POST" style="display: none;">
                            @csrf
                        </form>
                    </li>
                    <li class="nav-item {{ request()->routeIs('frontend.order.cart') ? 'active' : '' }}" style=" position: absolute;right: 0;margin-right: 170px;">
                        <a class="nav-link" href="{{ route('frontend.order.cart') }}" id="cart_quantity">
                            <i class="fa cart-badge" id="cart-quantity" style="font-size:24px;padding: 0" value=@php $total_qty = \App\Models\Cart::where(['user_id' => auth()->user()->id])->sum('product_qty');echo $total_qty;@endphp>&#xf07a;</i>Cart
                        </a>
                    </li>
                    {{--<li class="nav-item {{ request()->routeIs('frontend.faq') ? 'active' : '' }}">
                        <a class="nav-link" href="{{ route('frontend.faq') }}">
                            {{ __('FAQ') }}
                        </a>
                    </li>--}}
                    {{--<li class="nav-item {{ request()->routeIs('frontend.contact-us') ? 'active' : '' }}">
                        <a class="nav-link" href="{{ route('frontend.contact-us') }}">
                            {{ __('Contact us') }}
                        </a>
                    </li>--}}
                @endguest
